import {zahSodaPop} from './sodapop.mjs?v=conversion-10';
import {CHANNELS,OUTCOMES,channelDetail,outcomeDetail} from './channels.mjs?v=conversion-10';
import {actionCycle,actionAt,goalFill,ACTION_GOALS,CYCLE_MS} from './flow.mjs?v=conversion-10';
export {CHANNELS} from './channels.mjs?v=conversion-10';
...
export function mountSignals({main,paused=false}){
 ...
 function pulse(el,kind){
  ...
  const center=el===core?'translate(-50%,-50%) ':'';
  const scales=kind==='in'?[1,1.06,1]:kind==='out'?[1,.965,1.025,1]:[1,.94,1.06,1];
  if(typeof el.animate==='function'){
   el.animate(scales.map(n=>({transform:center+'scale('+n+')'})),{duration:kind==='tap'?480:350,easing:'cubic-bezier(.2,.75,.25,1)'});
   return;
  }
  const fallback='signal-pulse-'+kind;
  el.classList.remove('signal-pulse-in','signal-pulse-out','signal-pulse-tap');
  void el.offsetWidth;
 ...
 function reset(){jobs.forEach(j=>j.dot?.remove());clock=0;last=0;jobs=actionCycle();if(queuedSource!==null){jobs[0].source=queuedSource;queuedSource=null;}status.textContent=paused?'Effects are paused. Select Enable effects above to watch the journey move.':'Attention becomes useful context, then a visible customer outcome.';counts=[0,0,0];completed=[null,null,null];destinations.forEach((d,i)=>{d.classList.remove('delivered');d.dataset.goalMet='false';d.querySelector('.delivery-check').textContent=i+1;});lab.dataset.cycle=cycle;$('#signal-cycle').textContent='Cycle '+cycle;draw();}
 function dot(j,path,p){if(!j.dot){j.dot=add('circle',{class:'lead-dot',r:matchMedia('(max-width:760px)').matches?5:6.5,'data-source':CHANNELS[j.source].name},$('.signal-dots'));j.dot.style.setProperty('--source-color',CHANNELS[j.source].color);}if(path.hasAttribute('d')){const point=path.getPointAtLength(path.getTotalLength()*p);j.dot.setAttribute('cx',point.x);j.dot.setAttribute('cy',point.y);}}
 ...
 function draw(){let incoming=0,outgoing=0,processing=0;const activePaths=new Set();for(const j of jobs){if(j.arrived)continue;const state=actionAt(j,clock);if(!state.started)continue;j.started=true;if(state.entered&&!j.entered){j.entered=true;pulse(core,'in');}if(state.left&&!j.left){j.left=true;pulse(core,'out');}if(state.arrived){arrive(j);continue;}if(!state.entered){dot(j,paths[j.source],state.incoming);j.dot.dataset.leg='incoming';activePaths.add(j.source);incoming++;}else if(state.left){dot(j,paths[j.destination+4],state.outgoing);j.dot.dataset.leg='outgoing';activePaths.add(j.destination+4);outgoing++;}else{j.dot?.remove();j.dot=null;processing++;}}
 paths.forEach((path,i)=>path.classList.toggle('lit',activePaths.has(i)));
 ...
 return {setPaused(value){paused=Boolean(value);if(paused){stop();status.textContent='Effects are paused. Select Enable effects above to watch the journey move.';}else{status.textContent='Attention becomes useful context, then a visible customer outcome.';schedule();}}};
}
