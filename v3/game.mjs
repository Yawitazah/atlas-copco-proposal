import {STEPS,MODELS,blank,xp,preferred,complete,esc,localDate} from './mission.mjs?v=refinement-1';
import {QUESTIONS,questionsFor,questionError} from './questions.mjs?v=refinement-1';
import {mountEquipment} from './equipment.mjs?v=refinement-1';
...
export function mountGame({toast,paused,handoff,request,recordInfo,openRecord,go,onClear}){
 ...
 function header(){
  const points=xp(state),name=state.answers.first?.trim();
  $('#player-name').textContent=name?name+'’s equipment plan':'Let’s start with you.';$('#xp-total').textContent=points.toLocaleString()+' points';$('#badge-count').textContent=state.done.length+' of 7 sections';$('#xp-fill').style.width=points/10+'%';
  $('#live-xp').innerHTML='<span>'+esc(name||'Your plan')+'</span><b>'+points.toLocaleString()+' points</b>';
 ...
 function reward(i){clearTimeout(rewardTimer);const el=$('.survey-identity');if(!paused())zahSodaPop(el,false);$('#stage-reward').textContent='✓ '+STEPS[i].badge+' · +'+STEPS[i].xp+' points';rewardTimer=setTimeout(header,1800);}
 ...
 function render(){
  ...
  $('#stage-eyebrow').textContent=STEPS[q.group].label+' · '+(position+1)+' of '+groupQs.length;$('#stage-reward').textContent=state.done.includes(q.group)?'✓ Section completed':'+'+STEPS[q.group].xp+' points for this section';
  let body='';
  ...
  if(q.type==='equipment'){state.answers.model ||= preferred(state.answers);mountEquipment($('#garage'),state.answers,{paused,onSelect:id=>{state.answers.model=id;save();}});}
  const form=$('#mission-form');form.oninput=()=>{collect();header();save();};form.onchange=()=>{collect();form.querySelectorAll('.answer-choice').forEach(el=>el.querySelector('i').textContent=el.querySelector('input').checked?'✓':'+');save();};
 ...
 $('#restart-mission').onclick=()=>{if(!confirm('Clear your local profile and its demonstration sales record?'))return;onClear(state.id);state=blank();save();render();focusStage();toast('Ready for a new equipment profile.');};
 render();
